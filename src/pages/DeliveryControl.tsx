...
const DeliveryControl: React.FC = () => {
  ...
  const handleAssignAuto = async (orderId: number) => {
    try {
      ...
      toast({
        title: "Erro",
        description: error.response?.data?.message || 'Erro ao atribuir automaticamente',
        variant: "destructive"
      });
    } finally {
      setAssigningOrder(null);
          ...
          <div className="p-6">
            {Array.isArray(deliveryPeople) && deliveryPeople.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {(Array.isArray(deliveryPeople) ? deliveryPeople : []).map((delivery) => (
                  <div
                    ...
                    <ul className="space-y-2">
                      {deliveryOrders.map(order => (
                        <li key={`modal-order-${order.id}`} className="border rounded p-2">
                          ...
                          <div className="text-sm text-gray-700">Endereço: {order.customer_address}</div>
